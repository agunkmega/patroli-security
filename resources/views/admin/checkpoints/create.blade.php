...
@section('content')
<div class="max-w-2xl mx-auto space-y-6" x-data="{ latitude: '', longitude: '', getCurrentPosition() { if (navigator.geolocation) { navigator.geolocation.getCurrentPosition(pos => { this.latitude = pos.coords.latitude; this.longitude = pos.coords.longitude; }, err => { alert('Gagal akses lokasi: ' + err.message + '. Coba pakai localhost atau HTTPS.'); }, { enableHighAccuracy: true, timeout: 10000 }) } else { alert('Browser tidak mendukung GPS. Coba Chrome/Firefox terbaru.'); } } }">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Checkpoint</h1>
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Buat titik checkpoint baru</p>
    </div>

    <form method="POST" action="{{ route('admin.checkpoints.store') }}" class="bg-white dark:bg-dark-800 rounded-2xl p-6 border border-gray-200 dark:border-dark-700 space-y-5">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Kode *</label>
                <input type="text" name="code" value="{{ old('code') }}" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20" placeholder="CP-001">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nama *</label>
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20" placeholder="Pos Depan Utama">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="radius" value="{{ old('radius', 30) }}" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', 0) }}" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20">
            </div>
            <div>
                ...
                <label class="flex items-center gap-3 mt-2">
                    <input type="checkbox" name="require_photo" value="1" class="rounded border-gray-300 text-orange-500 focus:ring-orange-500/20">
                    <span class="text-sm text-gray-600 dark:text-gray-400">Ya, wajib upload foto</span>
                </label>
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Deskripsi</label>
                <textarea name="description" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20">{{ old('description') }}</textarea>
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Instruksi</label>
                <textarea name="instruction" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20" placeholder="Instruksi khusus di checkpoint ini">{{ old('instruction') }}</textarea>
            </div>
        </div>
