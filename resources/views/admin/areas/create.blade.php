...
@section('content')
<div class="max-w-2xl mx-auto" x-data="{ latitude: '', longitude: '', getCurrentPosition() { if (navigator.geolocation) { navigator.geolocation.getCurrentPosition(pos => { this.latitude = pos.coords.latitude; this.longitude = pos.coords.longitude; }, err => { alert('Gagal akses lokasi: ' + err.message + '. Coba pakai localhost atau HTTPS.'); }, { enableHighAccuracy: true, timeout: 10000 }) } else { alert('Browser tidak mendukung GPS.'); } } }">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Area</h1>
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Buat area patroli baru</p>
    </div>
    <form method="POST" action="{{ route('admin.areas.store') }}" class="bg-white dark:bg-dark-800 rounded-2xl p-6 border border-gray-200 dark:border-dark-700 space-y-5">
        @csrf
        <div class="grid grid-cols-2 gap-5">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Kode *</label>
                <input type="text" name="code" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500" placeholder="AR-001">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nama *</label>
                <input type="text" name="name" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500" placeholder="Area Utama">
            </div>
            <div>
        ...
        <div>
            ...
            <textarea name="address" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-dark-600 bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:border-orange-500"></textarea>
        </div>
